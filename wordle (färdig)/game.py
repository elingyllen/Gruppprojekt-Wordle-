...
def next_word():
    global TARGET, current_row, game_over
    
    TARGET = random.choice(WORDS)
    current_row = 0
    game_over = False

    # återställer alla rutor
    for r in range(ROWS):
        for c in range(COLS):
            grid[r][c].config(
                state="normal",
    ...
    # inaktiverar alla rader utom första
    for r in range(ROWS):
        for c in range(COLS):
...
def submit_guess(event=None):
    ...
    guess = "".join(grid[current_row][c].get().upper() for c in range(COLS))
    ...
    if guess == TARGET:
        game_over = True
        root.after(100, lambda: (messagebox.showinfo("You Win!", f"You guessed the word: {TARGET}"), next_btn.config(state="normal")))
        return
    ...
    if current_row == ROWS:
        game_over = True
        root.after(100, lambda: (messagebox.showinfo("Game Over", f"The word was: {TARGET}"), next_btn.config(state="normal")))
        return
...
def handle_key(event, r, c):
    ...
    if event.char.isalpha():
        grid[r][c].delete(0, tk.END)
        grid[r][c].insert(0, event.char.upper())
...
next_btn.grid(row=ROWS, column=0, columnspan=COLS, pady=10)

for c in range (COLS):
    grid[0][c].delete(0, tk.END)
    grid[0][c].config(state="normal")
